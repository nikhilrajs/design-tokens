...
const SelectTrigger = ({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "flex w-fit items-center justify-between gap-[var(--pcs-select-trigger-icon-gap)] rounded-[var(--pcs-select-trigger-border-radius)] border-[length:var(--pcs-select-trigger-border-width)] border-[color:var(--pcs-select-trigger-border-color)] bg-[var(--pcs-select-trigger-bg)] [box-shadow:var(--pcs-select-trigger-shadow)] py-2 px-[var(--pcs-select-trigger-md-padding-x)] text-[length:var(--pcs-select-trigger-md-font-size)] text-[color:var(--pcs-select-trigger-text)] whitespace-nowrap transition-colors outline-none select-none hover:border-[color:var(--pcs-select-trigger-border-color-hover)] focus-visible:border-[color:var(--pcs-select-trigger-border-color-open)] focus-visible:ring-[length:var(--pcs-focus-ring-width)] focus-visible:ring-[var(--pcs-color-focus-ring)]/50 disabled:pointer-events-none disabled:opacity-disabled disabled:bg-[var(--pcs-select-trigger-bg-disabled)] disabled:border-[color:var(--pcs-select-trigger-border-color-disabled)] disabled:text-[color:var(--pcs-select-trigger-text-disabled)] disabled:[box-shadow:var(--pcs-select-trigger-shadow-disabled)] disabled:[&_svg]:text-[color:var(--pcs-color-text-disabled)] aria-invalid:border-[color:var(--pcs-select-trigger-border-color-error)] aria-invalid:[box-shadow:var(--pcs-select-trigger-shadow-error-focus)] data-placeholder:text-[color:var(--pcs-select-trigger-placeholder)] data-[size=default]:h-[var(--pcs-select-trigger-md-height)] data-[size=sm]:h-[var(--pcs-select-trigger-sm-height)] *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
...
const SelectItem = ({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-[var(--pcs-menu-item-gap)] rounded-[var(--pcs-menu-item-border-radius)] py-[var(--pcs-menu-item-padding-y)] pr-8 pl-[var(--pcs-menu-item-padding-x)] text-[length:var(--pcs-menu-item-font-size)] outline-hidden select-none focus:bg-[var(--pcs-menu-item-bg-hover)] focus:text-[color:var(--pcs-menu-item-text-hover)] not-data-[variant=destructive]:focus:**:text-[color:var(--pcs-menu-item-text-hover)] data-disabled:pointer-events-none data-disabled:opacity-disabled [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className
      )}
      {...props}
    >
