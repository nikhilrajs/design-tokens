    ...
    <ComboboxPrimitive.Portal>
      ...
      >
        <ComboboxPrimitive.Popup
          ...
          data-chips={!!anchor}
          className={cn("group/combobox-content relative max-h-(--available-height) w-(--anchor-width) max-w-(--available-width) min-w-[calc(var(--anchor-width)+--spacing(7))] origin-(--transform-origin) overflow-hidden rounded-[var(--pcs-menu-border-radius)] bg-[var(--pcs-menu-bg)] text-[color:var(--pcs-menu-item-text)] [box-shadow:var(--pcs-menu-shadow)] border-[length:var(--pcs-menu-border-width)] border-[color:var(--pcs-menu-border-color)] duration-100 data-[chips=true]:min-w-(--anchor-width) data-[side=bottom]:slide-in-from-top-2 data-[side=inline-end]:slide-in-from-left-2 data-[side=inline-start]:slide-in-from-right-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 *:data-[slot=input-group]:m-1 *:data-[slot=input-group]:mb-0 *:data-[slot=input-group]:h-8 *:data-[slot=input-group]:border-input/30 *:data-[slot=input-group]:bg-input/30 *:data-[slot=input-group]:shadow-none data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95", className )}
          {...props}
        />
      </ComboboxPrimitive.Positioner>
    </ComboboxPrimitive.Portal>
...
const ComboboxList = ({ className, ...props }: ComboboxPrimitive.List.Props) => {
  return (
    <ComboboxPrimitive.List
      ...
      className={cn(
        "no-scrollbar max-h-[min(calc(--spacing(72)---spacing(9)),calc(var(--available-height)---spacing(9)))] scroll-py-1 overflow-y-auto overscroll-contain p-[var(--pcs-menu-padding)]",
        className
      )}
      {...props}
    />
...
const ComboboxItem = ({
  ...
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-[var(--pcs-menu-item-gap)] rounded-[var(--pcs-menu-item-border-radius)] py-[var(--pcs-menu-item-padding-y)] pr-8 pl-[var(--pcs-menu-item-padding-x)] text-[length:var(--pcs-menu-item-font-size)] outline-hidden select-none data-highlighted:bg-[var(--pcs-menu-item-bg-hover)] data-highlighted:text-[color:var(--pcs-menu-item-text-hover)] not-data-[variant=destructive]:data-highlighted:**:text-[color:var(--pcs-menu-item-text-hover)] data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
...
const ComboboxChip = ({
  ...
  return (
    <ComboboxPrimitive.Chip
      ...
      className={cn(
        "flex h-[calc(--spacing(5.25))] w-fit items-center justify-center gap-1 rounded-sm bg-[var(--pcs-color-surface-neutral-active)] px-1.5 text-xs font-medium whitespace-nowrap text-foreground has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50 has-data-[slot=combobox-chip-remove]:pr-0",
        className
      )}
      {...props}
    >
